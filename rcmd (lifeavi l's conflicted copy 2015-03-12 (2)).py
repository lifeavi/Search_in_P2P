...
class Assign4(threading.Thread):
		...
		while 1:
			...
			if not data: 
				...
				if "Okay will search for:" in retVal:
					d = data.split()
					self.command_to_send_nodes(d[1], d[2], d[3], d[4])
	...
	def command_to_send_nodes(self,command_to_send, fileName = None, hops = "0", ID = "0.0"):
		if fileName == None:
			x = command_to_send+" "+str(self.ip_address)+" "+str(int(self.port_no))
		else:
			x = command_to_send+" "+str(self.ip_address)+" "+str(int(self.port_no))+" "+fileName+" "+str(hops)+" "+str(ID)
		if len(x) < 100:
			leng = '00'+str(len(x)+5)
		elif len(x) > 99:
			leng = '0'+str(len(x)+5)
