...
class Assign4(threading.Thread):
	...
	def __init__(self, whichThread, ip_address, port_no, ipOfNeighbors, fileSystem):
	...
	def bind_ip(self,port_no):
		while 1:
			...
			if not data: 
				...
				if "Okay will search for:" in retVal:
					d = data.split()
					self.command_to_send_nodes(d[1], d[2], str(int(d[3])+1), d[4])
	...
	def searchForFile(self, IP, port, file_name):
		pass
	...
	def parseAction(self,action):
		...
		if actions[0] == "REG" or actions[0] == "UNREG":
			...
			if self.fileSystem.hasFile():
				print "I already have: " + self.fileSystem.getFile(actions[3])
			else:
				ID = str(time.time())
				self.listOfSearches.append(ID)
				self.command_to_send_nodes(actions[0], actions[3], "0", ID)
		elif actions[0].lower() == 'files':
			self.fileSystem.printFiles()
		else:
	...
	def command_to_send_nodes(self,command_to_send, fileName = None, hops = "0", ID = "0.0", ip ="", port =""):
		if fileName == None:
			x = command_to_send+" "+str(self.ip_address)+" "+str(int(self.port_no))
		elif ip == "":
			x = command_to_send+" "+str(self.ip_address)+" "+str(int(self.port_no))+" "+fileName+" "+hops+" "+ID
		else:
			x = command_to_send+" "+str(ip)+" "+str(int(port))+" "+fileName+" "+hops+" "+ID
		if len(x) < 100:
			leng = '00'+str(len(x)+5)
		elif len(x) > 99:
			leng = '0'+str(len(x)+5)
	...
	def parsedata(self, data):
		...
		if d[1] == "SER":
			if d[6] in self.listOfSearches:
				return "***"
			else:
				return "Okay will search for: "+ d[3]
	...
	def run(self):
		if self.whichThread == 0:
			...
			self.socks.bind((ip_address, 6337))
			self.sockr.bind((ip_address, port_no))
			self.bind_ip(port_no)
		else:
			self.socks = socket.socket(socket.AF_INET, socket.SOCK_DGRAM) # UDP
			self.socks.bind((ip_address, 6332))
			self.listenForArgs()
...
if __name__ == '__main__':		
	...
	port_no = int(raw_input("What port would you like to use: "))
	
	fileSystem = FileSystem()
	thread0 = Assign4(0, ip_address, port_no, ipOfNeighbors, fileSystem)
	thread1 = Assign4(1, ip_address, port_no, ipOfNeighbors, fileSystem)
